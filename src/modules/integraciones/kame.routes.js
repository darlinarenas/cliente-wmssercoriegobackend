...
kameRouter.post('/orders',async(req,res,next)=>{
 try{
  ...
  const now=new Date().toISOString(),id=`ORD-KAME-${Date.now()}`;const order={id,externalNumber,source:'KAME_API',sourceSiteId,requestSiteId,status:'RECIBIDA',assignedTo:null,items:[...grouped.values()],createdAt:now,createdBy:'KAME',events:[{at:now,userId:'KAME',message:'Orden recibida desde integración Kame'}]};
