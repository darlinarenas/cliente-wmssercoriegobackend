...
stateRouter.post('/reset',requireRole('ADMIN_GLOBAL','ADMINISTRADOR'),async(req,res,next)=>{try{const current=await readState(undefined,req.user);const fresh={...INITIAL_STATE,users:current.users,session:{userId:req.user.id},meta:{...INITIAL_STATE.meta,revision:current.meta.revision}};res.json(await withTransaction(c=>replaceState(c,fresh,current.meta.revision,req.user)));}catch(e){next(e);}});
